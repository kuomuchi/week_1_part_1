...
xhr.onreadystatechange = function(){
  if(xhr.readyState == 4){
    ...
    //放入color按鈕
    for(var i=0; i<objData.data.colors.length; i++){
      let outElement = document.getElementById('product_colors');
    ...
    //放入size按鈕
    for(var i=0; i<objData.data.sizes.length; i++){
      let outElement = document.getElementById('product_sizes');
      let addNewChild = document.createElement('div');
    ...
    //放入image
    for(var i=0; i<objData.data.images.length; i++){
      let outElement = document.getElementById('product');
      let addNewChild = document.createElement('img');
      addNewChild.classList.add('product_image');
      addNewChild.src = objData.data.images[i];
      outElement.appendChild(addNewChild);
    }
...
xhr.open('GET', `http://localhost:3000/api/1.0/products/details?id=${productId}`);
xhr.send();